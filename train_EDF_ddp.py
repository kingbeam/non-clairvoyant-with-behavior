"""
DDP — Preemptive EDF-BC for max-stretch.
Oracle: EDF at S* (optimal for max-stretch via bisection).
BC: learn to pick the job with earliest deadline at each decision point.
"""
...
CFG = dict(
    data_path    = "/root/non-clairvoyant-with-behavior/data/train_online.csv",
    test_path    = "/root/non-clairvoyant-with-behavior/data/val_online.csv",
    feature_cfg  = "/root/non-clairvoyant-with-behavior/data/feature_config.json",
...
def main():
    ...
    agent = SchedulerAgent("/root/non-clairvoyant-with-behavior/checkpoints/encoder_best.pt", device=f"cuda:{local_rank}", random_init=True)
    ...
    _ck_meta = torch.load("/root/non-clairvoyant-with-behavior/checkpoints/encoder_best.pt", map_location="cpu", weights_only=False)
        ...
        if is_main_process(rank):
            ...
            if np.isfinite(m["rho_max"]) and m["rho_max"] < best_rho_max:
                ...
                torch.save(dict(ptr_head=ptr_head_ddp.module.state_dict(),
                           ...
                           "/root/non-clairvoyant-with-behavior/checkpoints/policy_edf_best.pt")
            ...
            with open("/root/non-clairvoyant-with-behavior/eval_edf.log", "a") as f:
                f.write(line + "\n")
    ...
    if is_main_process(rank):
        ...
        torch.save(dict(ptr_head=ptr_head_ddp.module.state_dict(),
                        encoder=encoder_ddp.module.state_dict()),
                   "/root/non-clairvoyant-with-behavior/checkpoints/policy_edf_final.pt")
