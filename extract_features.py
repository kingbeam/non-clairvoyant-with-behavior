"""
Feature extraction for the GPU job trace.

Loads job_info_df.csv, engineers per-job numeric and categorical features
(resource requests, submission-time patterns, per-user histories, and quantile
signatures), and writes train/val/test splits plus feature_config.json.
...
BASE_DIR = pathlib.Path(__file__).resolve().parent  # .../data_gpu_trace/
...
def load_jobs():
    ...
    # Drop persistent jobs already running when the trace starts (submit_time=0, 3,588 on-demand):
    # their duration records an end time rather than processing time, so they are excluded
    # from both the time-pattern analysis and the data (consistent with plot_regularity.py).
    df = df[df["submit_time"] > 0].reset_index(drop=True)
    # per-worker requests -> job-level totals
...
def add_basic_features(df):
    ...
    # CPU:GPU balance - CPU-bound vs GPU-bound job profile (GPU-trace specific).
    ...
    # to d5-d6 after +4, matching the ALIGN computed in plot_regularity.py.
