"""
Evaluate the learned scheduling policy on the makespan objective.

Loads the trained encoder and pointer head, then runs an RL rollout that assigns
each job to the currently least-loaded machine (perceived load from predicted
processing times). The resulting makespan is compared against a clairvoyant LPT
baseline and a lower bound, across several machine counts.

Usage:
    python eval_lpt_makespan.py [checkpoint] [m1 m2 ...]
"""
...
def log(msg):
    ...
    with open(os.path.join(_ROOT, 'eval_lpt_makespan.log'), 'a') as f:
...
def main():
    ...
    for m in MACHINE_COUNTS:
        log(f"  m={m:>3d} (n={results[m]['n']:>5d})  RL ρ={results[m]['rl_rho']:.3f}  true-LPT ρ={results[m]['lpt_rho']:.3f}")
    log(f"  avg_RL={avg:.4f}  avg_true-LPT={lpt_avg:.4f}  | elapsed {time.time()-t0:.0f}s")
